      ...
      >
        ...
        <div className="hidden items-center gap-0.5 rounded-2xl border border-slate-200/80 bg-slate-50/80 p-1.5 shadow-inner lg:flex">
          {navigationItems.map(({ label, path, section, icon: Icon }) => (
            <button
              key={path || section}
              type="button"
              onClick={() => (path ? goTo(path) : scrollToSection(section!))}
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <Button
            type="button"
            onClick={() => goTo("/signup")}
            className="h-11 rounded-xl bg-gradient-to-br from-emerald-600 to-emerald-700 px-5 text-base font-semibold text-white shadow-[0_8px_20px_rgba(0,158,96,0.22)] transition-all hover:-translate-y-0.5 hover:from-emerald-500 hover:to-emerald-700 hover:shadow-[0_10px_24px_rgba(0,158,96,0.28)]"
          >
            Get started
          ...
          <Button
            ...
            variant="ghost"
            onClick={() => goTo("/login")}
            className="h-11 rounded-xl px-4 text-base font-semibold text-slate-700 hover:bg-slate-100 hover:text-slate-950"
          >
            ...
            <div className="flex h-[calc(100%-89px)] flex-col px-4 py-5">
              ...
              <div className="mt-auto grid gap-2 border-t border-slate-100 pt-5">
                <Button
                  type="button"
                  onClick={() => goTo("/signup")}
                  className="h-11 rounded-xl font-semibold text-white shadow-lg shadow-emerald-900/10"
                >
                  Start free trial
                  <ArrowRight className="size-4" />
                </Button>
